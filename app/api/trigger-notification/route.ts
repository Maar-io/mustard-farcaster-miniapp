...
export async function POST(req: NextRequest) {
  ...
  if (!details) {
    return NextResponse.json(
      { error: `No notification token registered for address ${body.userAddress}` },
      { status: 404 },
    );
  }
  ...
  try {
    await sendNotification(details.url, {
      notificationId: `mustard-external-${Date.now()}`,
      title: 'Mustard',
      body: 'Hello from Mustard! This notification was triggered externally.',
      targetUrl: appUrl,
      tokens: [details.token],
    });

    console.log(`[trigger-notification] sent to ${body.userAddress} via ${details.url}`);
    return NextResponse.json({ success: true });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Unknown error';
    console.error('[trigger-notification] failed:', msg);
